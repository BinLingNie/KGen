...
def generate_batch(batch):
	text = [F.pad(torch.tensor(x[0]), (0, max_seq_len - len(x[0])), "constant", 1) for x in
	        batch]  # batch_size * max_seq_len
	text = pad_sequence(text, batch_first=True)
	attention_mask = [
		torch.cat((torch.ones_like(torch.tensor(x[0])), torch.zeros(max_seq_len - len(x[0]), dtype=torch.int64)), dim=0)
		if len(x[0]) < max_seq_len else torch.ones_like(torch.tensor(x[0]))[:max_seq_len] for x in batch]
	attention_mask = pad_sequence(attention_mask, batch_first=True)
	label = [F.pad(torch.tensor(x[1]), (0, max_seq_len - len(x[1])), "constant", REPLACE_TAG) for x in batch]
	label = pad_sequence(label, batch_first=True)
	ent = [F.pad(torch.tensor(x[2]), (0, max_seq_len - len(x[2])), "constant", 0) for x in batch]
	ent = pad_sequence(ent, batch_first=True)
	orig_len = [len(x[0]) for x in batch]
	
	return text, attention_mask, label, ent, orig_len
...
def train_func(processed_training_set, epoch, tokenizer, label_sentence_dicts, soft_kmeans, count_num=0,
	...
	for k in all_data_index:
		...
		outputs = output
		optimizer.step()
		preds = [[id2label[int(x)] for j, x in enumerate(y[1:orig_len[i] - 1]) if int(cls[i][j + 1]) != REPLACE_TAG] for i, y
		         in enumerate(outputs)]
		gold = [[id2label[int(x)] for x in y[1:orig_len[i] - 1] if int(x) != REPLACE_TAG] for i, y in enumerate(cls)]
		...
		if count_num % 200 == 0:
			print(
				f"batch: {count_num}/{int(num_training_steps / N_EPOCHS)} lr: {optimizer.param_groups[0]['lr']:.9f} loss: {loss.mean().item() / BATCH_SIZE:.9f}")
...
def test(data_, epoch, label_sentence_dicts, soft_kmeans, test_all_data=True, finetune=False):
	...
	if test_all_data:
		...
		data, _ = generate_episode(data_, epoch, EPISODE_NUM, TEST_SUP_CLS_NUM, test_id2label, label_sentence_dict,
		                           use_multipledata=False)
	idx = 0
	f1ss = []
	pss = []
	rss = []
	...
	for j, (text, attention_mask, cls, ent_cls, orig_len) in enumerate(data):
	...
	for x in total_pred_per_type:
		...
		microf1_per_type[x] = 2 * microp_per_type[x] * micror_per_type[x] / (
					microp_per_type[x] + micror_per_type[x]) if (microp_per_type[x] + micror_per_type[x]) > 0 else 0
...
if __name__ == "__main__":
	...
	for round in range(args.few_shot_sets):
		if '.' not in args.train_text:
			...
			args.train_ent = train_ent_file + f'_{round}.ent'
		...
		train_ents = [os.path.join(args.datapath, dataset, args.train_ent) for dataset in datasets]
		...
		test_ents = [os.path.join(args.datapath, dataset, args.test_ent) for dataset in datasets]
		...
		if not args.load_dataset:
			for train_text, train_ner, train_ent, test_text, test_ner, test_ent in zip(train_texts, train_ners,
			                                                                           train_ents, test_texts,
			                                                                           test_ners, test_ents):
				with open(train_ner) as fner, open(train_text) as f, open(train_ent) as fent:
					train_ner_tags, train_words, train_ents = fner.readlines(), f.readlines(), fent.readlines()
				with open(test_ner) as fner, open(test_text) as f, open(test_ent) as fent:
					test_ner_tags, test_words, test_ents = fner.readlines(), f.readlines(), fent.readlines()
				label2id, id2label = get_label_dict([train_ner_tags, test_ner_tags])
				
				label2ids.append(label2id)
				id2labels.append(id2label)
				
				train_ner_tags, train_ents, train_words, train_label_sentence_dict = process_data(train_ner_tags, train_ents, train_words,
				                                                                      tokenizer, label2id, max_seq_len,
				                                                                      base_model=base_model,
				                                                                      use_truecase=args.use_truecase)
				test_ner_tags, test_ents, test_words, test_label_sentence_dict = process_data(test_ner_tags, test_ents, test_words, tokenizer,
				                                                                   label2id, max_seq_len,
				                                                                   base_model=base_model,
				                                                                   use_truecase=args.use_truecase)
				
				sub_train_ = [[train_words[i], train_ner_tags[i], train_ents[i]] for i in
				              range(len(train_ner_tags))]
				sub_valid_ = [[test_words[i], test_ner_tags[i], test_ents[i]] for i in range(len(test_ner_tags))]
		...
		if LOAD_MODEL:
			if 'checkpoint' not in args.load_model_name:
				...
				if base_model == 'roberta':
					model = RobertaNER.from_pretrained('../roberta/', dataset_label_nums=dataset_label_nums,
					                                   output_attentions=False, output_hidden_states=False,
					                                   multi_gpus=True)
				elif base_model == 'bert':
					model = BertNER.from_pretrained('bert-base-cased', dataset_label_nums=dataset_label_nums,
					                                output_attentions=False, output_hidden_states=False,
					                                multi_gpus=True)
				...
				pretrained_dict = state['state_dict']
				model_dict = model.state_dict()
				pretrained_dict = {k.split('module.')[1]: v for k, v in pretrained_dict.items() if
				                   k.split('module.')[1] in model_dict and 'classifier' not in k}
			...
			model.classifiers = torch.nn.ModuleList(
				[torch.nn.Linear(model.config.hidden_size, x) for x in dataset_label_nums])
		else:
			if base_model == 'roberta':
				model = RobertaNER.from_pretrained('../roberta/', dataset_label_nums=dataset_label_nums,
				                                   output_attentions=False, output_hidden_states=False, multi_gpus=True)
			elif base_model == 'bert':
				model = BertNER.from_pretrained('bert-base-cased', dataset_label_nums=dataset_label_nums,
				                                output_attentions=False, output_hidden_states=False, multi_gpus=True)
		if args.reinit:
		...
		num_training_steps = N_EPOCHS * int(
			sum([len(sub_train_) for sub_train_ in processed_training_set]) / BATCH_SIZE)
		...
		if LOAD_CHECKPOINT:
			...
			secs = secs % 60
			print(
				f"loaded from checkpoint - learning rate: {optimizer.param_groups[0]['lr']:.9f} | time in {mins} minutes, {secs} seconds")
		...
		for epoch in range(start_epoch, N_EPOCHS):
			...
			if args.load_checkpoint and epoch == start_epoch:
				train_loss, microp, micror, microf1 = train_func(processed_training_set, epoch, tokenizer,
				                                                 train_label_sentence_dicts, soft_kmeans=SOFT_KMEANS,
				                                                 count_num=start_count_num)
			else:
				train_loss, microp, micror, microf1 = train_func(processed_training_set, epoch, tokenizer,
				                                                 train_label_sentence_dicts, soft_kmeans=SOFT_KMEANS)
			
			print(
				f'\tLoss: {train_loss:.4f}(train)\t|\tPrec: {microp * 100:.1f}%(train)\t|\tRecall: {micror * 100:.1f}%(train)\t|\tF1: {microf1 * 100:.1f}%(train)')
			...
			valid_loss, microp, micror, microf1, microp_per_type, micror_per_type, microf1_per_type = test(
				processed_test_set, epoch, test_label_sentence_dicts, soft_kmeans=SOFT_KMEANS)
			print(
				f'\tLoss: {valid_loss:.4f}(val)\t|\tPrec: {microp * 100:.2f}%(val)\t|\tRecall: {micror * 100:.2f}%(val)\t|\tF1: {microf1 * 100:.2f}%(val)')
