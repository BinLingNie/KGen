...
def generate_episode(data_list, epoch, EPISODE_NUM, max_seq_len, sup_cls_num, SUP_PER_CLS, QUR_PER_CLS, id2labels, label_sentence_dicts, 
    ...
    for epi in range(int(episode_num/gpu_num)):
        ...
        if train_other_labels:
            ...
            inv_support_classes = defaultdict(lambda:REPLACE_TAG)
        for i,cls in enumerate(support_class):
            inv_support_classes[cls] = i
        inv_support_classes[REPLACE_TAG]=REPLACE_TAG
        idx = 0
        support_class = {idx:0}  # size: SUP_CLS_NUM * 2
        support_class[REPLACE_TAG] = REPLACE_TAG
        for cls in support_class_root:
            idx += 1
        ...
        for epi_iter in range(gpu_num):
            ...
            for cls in support_class_root:
                ...
                label.extend([F.pad(torch.tensor([inv_support_classes[y] for y in data[x][1]]), (0,max_seq_len-len(data[x][1])), "constant", REPLACE_TAG) \
...
def process_data(ner_tags, ent_tags, sents, tokenizer, label2id, max_sequence_len, base_model='roberta', lower=False, use_truecase=False):
    logging.info("Start processing raw data!")
    sent_words, sent_wpcs, sent_tags, sent_ents = [], [], [], []
    max_seq_len = 0
    ...
    for k,line in enumerate(sents):
        ...
        # [(1, 2), (2, 4), (4, 5)]
        sent_tag = [REPLACE_TAG]
        for i in range(len(aligns)):
            sent_tag.extend([label2id[tags[i]] if j == 0 else REPLACE_TAG for j in range(aligns[i][1]-aligns[i][0])])
            #sent_tag.extend([label2id[tags[i]] if j == 0 else 0 for j in range(aligns[i][1] - aligns[i][0])])
            # if tags[i] == 'O':
        ...
        sent_ent = [0]
        for i in range(len(aligns)):
            sent_ent.extend([label2id[e_tags[i]] if j == 0 else 0 for j in range(aligns[i][1] - aligns[i][0])])
            # sent_tag.extend([label2id[tags[i]] if j == 0 else 0 for j in range(aligns[i][1] - aligns[i][0])])
            # if tags[i] == 'O':
            #     sent_tag.extend([label2id[tags[i]] for j in range(aligns[i][1]-aligns[i][0])])
            # else:
    ...
    return sent_tags, sent_ents, sent_wpcs, label_sentence_dict
